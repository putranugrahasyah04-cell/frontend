...
function getStatusInfo(penyakit, tingkat) {
  // Gunakan String() agar aman dari nilai apapun (null, number, object, dll)
  const p = String(penyakit || "").toLowerCase().trim();
  const t = String(tingkat  || "").toLowerCase().trim();

  // BUG FIX: Hapus "t === ''" dari isSehat.
  // Sebelumnya jika tingkat_keparahan kosong (""), item langsung dianggap Sehat,
  // padahal penyakitnya bisa berisi nama penyakit yang nyata.
  // Sekarang keputusan "Sehat" HANYA dari field penyakit.
  const isSehat =
    !p ||
  ...
  // Catch-all: semua yang tidak sehat dan tidak kritis → Perlu Perawatan
...
function AreaChart({ data }) {
  ...
  // ── FIX: Deteksi hover lewat mouse move di seluruh area SVG ──────────────
  // Tidak lagi bergantung pada onMouseEnter per titik — semua area bisa di-hover
  function handleMouseMove(e) {
    const svg = svgRef.current;
    if (!svg) return;
    const rect = svg.getBoundingClientRect();
    // Konversi koordinat mouse ke koordinat SVG viewBox
    const scaleX = W / rect.width;
    const mouseX = (e.clientX - rect.left) * scaleX;

    // Cari titik terdekat berdasarkan posisi X
    let closest = null;
    ...
    if (closest) {
      // Hitung posisi tooltip dalam persen relatif terhadap container SVG (untuk HTML overlay)
        ...
        <>
          <div className="flex flex-wrap gap-x-5 gap-y-2 mb-4">
            <div className="flex items-center gap-2">
              <span className="flex items-center gap-1.5">
                <svg width="24" height="10"><line x1="0" y1="5" x2="24" y2="5" stroke="#16a34a" strokeWidth="2.5" strokeLinecap="round"/></svg>
              </span>
              <div>
                <span className="text-xs font-bold text-gray-700">Total Analisis</span>
                <span className="text-xs text-gray-400 ml-1">— semua pemeriksaan dilakukan tiap bulan</span>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <span className="flex items-center gap-1.5">
                <svg width="24" height="10"><line x1="0" y1="5" x2="24" y2="5" stroke="#10b981" strokeWidth="2" strokeDasharray="5 3" strokeLinecap="round"/></svg>
              </span>
              <div>
                <span className="text-xs font-bold text-gray-700">Tanaman Sehat</span>
                <span className="text-xs text-gray-400 ml-1">— dari total analisis yang hasilnya sehat</span>
              </div>
            </div>
            <div className="flex items-center gap-1.5 ml-auto">
              ...
              <span className="text-[10px] text-gray-400">otomatis diperbarui tiap bulan</span>
            </div>
          </div>
          ...
          <div className="relative">
            ...
            >
              ...
              {/* Titik data — selalu render, highlight saat aktif */}
              {pts.map((p, i) => (
                <g key={i}>
                  {p.d.total > 0 && (
                    <>
                  ...
                  {/* Titik kosong (tidak ada data) tetap tampil kecil */}
            ...
            {/* ── Tooltip HTML overlay — jauh lebih cantik dari SVG text ── */}
            {tooltip && (() => {
              ...
              // Posisi tooltip: ikut titik aktif, tapi jangan keluar layar
              // pctX dalam 0-100, kita gunakan sebagai left%
              const leftPct = Math.min(Math.max(tooltip.pctX, 12), 78);
              const isLeft = tooltip.pctX > 60; // kalau kursor di kanan, tooltip ke kiri
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    transform: isLeft ? "translateX(-105%)" : "translateX(8px)",
                    transition: "left 0.12s cubic-bezier(0.4,0,0.2,1), top 0.12s cubic-bezier(0.4,0,0.2,1)",
                  }}
                >
                  <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden min-w-[148px]"
                    style={{ boxShadow: "0 8px 32px -4px rgba(16,163,74,0.18), 0 2px 8px -2px rgba(0,0,0,0.08)" }}>

                    {/* Header dengan gradient */}
                    <div className="bg-gradient-to-r from-green-600 to-emerald-500 px-3.5 py-2.5">
                    ...
                    {tooltip.data.total > 0 && (
                      <div className="px-3.5 py-2.5 space-y-2">
                        ...
                        <div className="flex items-center justify-between gap-4">
                          ...
                          <span className="text-[13px] font-extrabold text-gray-800">{tooltip.data.total}</span>
                        </div>
                        ...
                        <div className="flex items-center justify-between gap-4">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-[11px] text-gray-500">Sehat</span>
                          </div>
                          <span className="text-[13px] font-extrabold text-emerald-600">{tooltip.data.sehat}</span>
                        </div>
                        ...
                        {sakit > 0 && (
                          <div className="flex items-center justify-between gap-4">
                            <div className="flex items-center gap-1.5">
                              ...
                              <span className="text-[11px] text-gray-500">Bermasalah</span>
                            </div>
                            <span className="text-[13px] font-extrabold text-red-500">{sakit}</span>
                          </div>
                        )}
                        ...
                        <div className="pt-1">
                          <div className="flex items-center justify-between mb-1">
                            <span className="text-[10px] text-gray-400">Tingkat Kesehatan</span>
                            <span className="text-[10px] font-bold text-emerald-600">{pctSehat}%</span>
                          </div>
          ...
          <div className="flex items-center gap-1 mt-3 pt-3 border-t border-gray-50">
            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mr-2">Bulan ini:</span>
            <div className="flex gap-3">
              <span className="inline-flex items-center gap-1.5 bg-green-50 border border-green-100 rounded-lg px-2.5 py-1">
                <span className="w-1.5 h-1.5 rounded-full bg-green-600" />
                <span className="text-xs text-gray-600">Total <strong className="text-gray-800">{current.total}</strong></span>
              </span>
              <span className="inline-flex items-center gap-1.5 bg-emerald-50 border border-emerald-100 rounded-lg px-2.5 py-1">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
                <span className="text-xs text-gray-600">Sehat <strong className="text-emerald-700">{current.sehat}</strong></span>
              </span>
              {currentSakit > 0 && (
                <span className="inline-flex items-center gap-1.5 bg-red-50 border border-red-100 rounded-lg px-2.5 py-1">
                  <span className="w-1.5 h-1.5 rounded-full bg-red-400" />
                  <span className="text-xs text-gray-600">Bermasalah <strong className="text-red-600">{currentSakit}</strong></span>
                </span>
              )}
            </div>
          </div>
...
// ── Fungsi ini tidak dipakai lagi (diganti Firestore), tapi dibiarkan
// sebagai fallback jika Firestore gagal load
...
export default function Monitoring() {
  ...
  // ── State utama: data dari Firestore ─────────────────────────────────────
  ...
  useEffect(() => {
    ...
    // FIX UTAMA: Gunakan parameter onError baru di listenRiwayat.
    // Sebelumnya error handler memanggil callback([]) yang mengosongkan history
    // sehingga semua stat jadi 0 / data "berubah-rubah" saat ada error Firestore.
    // Sekarang: error hanya menghentikan loading, data existing TETAP dipertahankan.
  ...
  // ── Hapus satu riwayat dari Firestore ─────────────────────────────────────
  async function handleDeleteItem(item) {
    if (!user?.uid) return;
    setDeleteLoading(true);
    try {
      await deleteRiwayat(user.uid, item.id);
      // Firestore onSnapshot akan otomatis update history
      setDeleteTarget(null);
      if (selectedItem?.id === item.id) setSelectedItem(null);
    } catch (err) {
      console.error("Gagal hapus:", err);
      // BUG FIX: Beri feedback ke user jika hapus gagal (sebelumnya silent)
  ...
  // ── BUG FIX: Hitung semua stats satu kali dari sumber yang sama ──────────
  // FIX: useMemo menjamin semua stat (total, sehat, luas, dll) dihitung
  // dalam satu pass dari snapshot history yang sama, tidak bisa inkonsisten.
  const stats = useMemo(() => {
    ...
    // parseFloat + String() menangani luas tersimpan sebagai number maupun string
    ...
    <div className="min-h-screen bg-gray-50/80 text-gray-800">
      ...
      <div className="bg-gradient-to-br from-green-600 via-green-600 to-emerald-700 px-6 py-10 relative overflow-hidden">
        ...
        <div className="max-w-screen-xl mx-auto relative z-10">
          <p className="text-green-200 text-sm mb-1">Dashboard Kebun</p>
          <h1 className="text-2xl md:text-3xl font-bold text-white">Monitoring Tanaman Sawit</h1>
      ...
      <div className="max-w-screen-xl mx-auto px-4 md:px-8 py-8 space-y-6">
        ...
        {/* STAT CARDS */}
        {/* FIX: Banner error Firestore - ditampilkan tanpa menghapus data */}
        {firestoreError && (
        ...
        {/* FIX: Skeleton loading untuk stat cards - sebelumnya cards menampilkan 0
            saat data Firestore belum tiba, sekarang tampil animasi skeleton dulu */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4" style={fadeUp(0)}>
          {loadingData ? (
            // Skeleton placeholder - muncul selama data Firestore dimuat
            <>
              {[0,1,2,3].map(i => (
                <div key={i} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex flex-col gap-3 animate-pulse">
                  ...
                  <div className="h-7 w-20 bg-gray-50 rounded" />
                </div>
              ))}
            </>
          ) : (
            <>
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex flex-col gap-3 relative overflow-hidden group hover:shadow-md transition-all duration-300 hover:-translate-y-0.5 stat-card-enter" style={{ animationDelay: "0ms" }}>
                ...
                <div className="flex items-center justify-between relative z-10">
                  <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Total Analisis</span>
                  <span className="w-9 h-9 bg-green-50 rounded-xl flex items-center justify-center text-sm shadow-sm border border-green-100">{"🔍"}</span>
                </div>
                <div className="relative z-10">
                  <p className="text-3xl font-extrabold text-gray-900 leading-none">{total}</p>
                  <p className="text-xs text-gray-400 mt-1">pemeriksaan tercatat</p>
                </div>
                <Sparkline data={sparkTotal} color="#16a34a" />
              </div>

              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex flex-col gap-3 relative overflow-hidden group hover:shadow-md transition-all duration-300 hover:-translate-y-0.5 stat-card-enter" style={{ animationDelay: "80ms" }}>
                ...
                <div className="flex items-center justify-between relative z-10">
                  <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Tanaman Sehat</span>
                  <span className="w-9 h-9 bg-emerald-50 rounded-xl flex items-center justify-center text-sm shadow-sm border border-emerald-100">{"💚"}</span>
                </div>
                <div className="relative z-10">
                  <p className="text-3xl font-extrabold text-emerald-600 leading-none">{sehat}</p>
                  <p className="text-xs text-gray-400 mt-1">{pctSehat}% dari total</p>
                </div>
                <Sparkline data={sparkData} color="#10b981" />
              </div>

              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex flex-col gap-3 relative overflow-hidden group hover:shadow-md transition-all duration-300 hover:-translate-y-0.5 stat-card-enter" style={{ animationDelay: "160ms" }}>
                ...
                <div className="flex items-center justify-between relative z-10">
                  <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Perlu Perawatan/Tindakan</span>
                  <span className="w-9 h-9 bg-red-50 rounded-xl flex items-center justify-center text-sm shadow-sm border border-red-100">{"⚠️"}</span>
                </div>
                <div className="relative z-10">
                  <p className="text-3xl font-extrabold text-red-500 leading-none">{sakit}</p>
                  <p className="text-xs text-gray-400 mt-1">{pctSakit}% dari total</p>
                </div>
                <Sparkline data={sparkSakit} color="#ef4444" />
              </div>

              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex flex-col gap-3 relative overflow-hidden group hover:shadow-md transition-all duration-300 hover:-translate-y-0.5 stat-card-enter" style={{ animationDelay: "240ms" }}>
                ...
                <div className="flex items-center justify-between relative z-10">
                  <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Lahan Dipantau</span>
                  <span className="w-9 h-9 bg-teal-50 rounded-xl flex items-center justify-center text-sm shadow-sm border border-teal-100">{"🌿"}</span>
                </div>
                <div className="relative z-10">
                  <p className="text-3xl font-extrabold text-teal-600 leading-none">{luasTotal} <span className="text-lg font-semibold">Ha</span></p>
                  <p className="text-xs text-gray-400 mt-1">area teranalisis</p>
                </div>
                <div className="h-1.5 rounded-full bg-gray-100 overflow-hidden">
        ...
        {/* CHART + RINGKASAN */}
        <div className="grid md:grid-cols-3 gap-4" style={fadeUp(150)}>
          <div className="md:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            <div className="mb-4">
              <h3 className="font-bold text-gray-900 text-base">Aktivitas Analisis Bulanan</h3>
              <p className="text-xs text-gray-400 mt-0.5">12 bulan terakhir · hover titik untuk lihat detail angka</p>
            </div>
            <AreaChart data={monthlyFull} />
          </div>

          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 flex flex-col">
            <h3 className="font-bold text-gray-900 text-base mb-5">Ringkasan Kondisi</h3>
            <div className="flex items-center justify-center mb-5">
              <RingChart sehat={sehat} sakit={sakit} total={total} />
            </div>
            <div className="space-y-2.5 mt-auto">
              {[
                // BUG FIX: Pakai nilai precomputed agar konsisten dengan stat cards
                ...
                <div key={item.label} className={`flex items-center justify-between ${item.bg} rounded-xl px-3 py-2.5 transition-all hover:scale-[1.01]`}>
                  <div className="flex items-center gap-2">
        ...
        {/* RIWAYAT */}
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden" style={fadeUp(250)}>
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-6 py-4 border-b border-gray-50">
            <div>
              <h3 className="font-bold text-gray-900 text-base">Riwayat Monitoring</h3>
              <p className="text-xs text-gray-400 mt-0.5">Data dari hasil analisis AI Cek Tanaman</p>
            </div>
            ...
            <>
              ...
              <div className="divide-y divide-gray-50/80">
                {displayed.map((item, i) => {
                  ...
                  return (
                    <div key={item.id || i}
                      className="grid grid-cols-1 md:grid-cols-[1fr_1fr_2fr_136px_52px] gap-2 md:gap-4 px-6 py-4 text-sm transition-all duration-200 hover:bg-green-50/30 row-hover group items-center"
              ...
              {filtered.length > 5 && (
                <div className="text-center px-6 py-4 border-t border-gray-50">
                  <button onClick={() => setShowAll(!showAll)}
                    className="text-sm font-semibold text-green-600 hover:text-green-700 bg-green-50 hover:bg-green-100 px-6 py-2 rounded-xl transition-all">
                    {showAll ? "Sembunyikan" : `Lihat ${filtered.length - 5} riwayat lainnya`}
                  </button>
                </div>
              )}
            </>
        ...
        <div style={fadeUp(350)}>
          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3 px-1">Konteks Industri Kelapa Sawit Indonesia</p>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {facts.map((f, i) => (
              <div key={i} className={`bg-gradient-to-br ${f.color} rounded-2xl p-5 text-white relative overflow-hidden group hover:scale-[1.02] transition-all duration-300 cursor-default`}>
                ...
                <p className="text-[10px] font-bold text-white/60 uppercase tracking-widest mb-2">{f.label}</p>
                <p className="text-2xl font-extrabold leading-none">{f.value}</p>
                <p className="text-xs text-white/60 mt-1.5">{f.sub}</p>
              </div>
            ))}
          </div>
          <p className="text-xs text-gray-400 mt-2 px-1">Sumber: GAPKI, Kementerian Pertanian RI, BPS 2023.</p>
        </div>

        {/* CTA */}
        <div className="bg-gradient-to-r from-green-600 to-emerald-600 rounded-2xl p-6 flex flex-col sm:flex-row items-center justify-between gap-4 relative overflow-hidden"
          style={fadeUp(400)}>
          <div className="absolute inset-0 pointer-events-none opacity-10"
            style={{ backgroundImage: "radial-gradient(circle, white 1px, transparent 1px)", backgroundSize: "20px 20px" }} />
          <div className="relative z-10">
            <h3 className="text-white font-bold text-lg">Mulai pemeriksaan baru sekarang</h3>
            <p className="text-green-100 text-sm mt-0.5">Deteksi dini penyakit sawit dengan AI untuk hasil panen optimal</p>
          </div>
          <a href="/cek" className="relative z-10 flex-shrink-0 bg-white text-green-700 font-bold text-sm px-6 py-3 rounded-xl hover:bg-green-50 transition-all shadow-sm hover:shadow-md hover:scale-105">
            Cek Tanaman
          </a>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4"
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto"
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h2 className="font-bold text-gray-900 text-base">Detail Hasil Analisis</h2>
            ...
            <div className="px-6 py-5 space-y-5">
              <div className="flex gap-4">
                {selectedItem.imagePreview ? (
                  <img src={selectedItem.imagePreview} alt="Foto tanaman" className="w-28 h-28 object-cover rounded-xl border border-gray-100 flex-shrink-0" />
                ) : (
                  <div className="w-28 h-28 rounded-xl bg-gradient-to-br from-green-50 to-emerald-100 flex items-center justify-center text-3xl flex-shrink-0 border border-green-100">{"🌿"}</div>
                )}
                <div className="flex-1 min-w-0">
                  {(() => {
                    ...
                    return (
                      <>
                        <div className="mb-2"><StatusBadge label={st.label} /></div>
            ...
            <div className="px-6 pb-5">
              <button onClick={() => setSelectedItem(null)}
                className="w-full py-2.5 bg-gray-100 text-gray-600 font-semibold text-sm rounded-xl hover:bg-gray-200 transition">
                Tutup
              </button>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6"
            ...
            <div className="flex gap-3">
              <button onClick={() => setDeleteTarget(null)}
                className="flex-1 py-2.5 bg-gray-100 text-gray-600 font-semibold text-sm rounded-xl hover:bg-gray-200 transition">
                Batal
              </button>
              <button onClick={() => handleDeleteItem(deleteTarget)}
                disabled={deleteLoading}
                className="flex-1 py-2.5 bg-red-500 text-white font-semibold text-sm rounded-xl hover:bg-red-600 transition shadow-sm disabled:opacity-60 disabled:cursor-not-allowed">
